...
function App() {
    ...
    setGame((current) => {
      ...
      const updatedGame = {
        ...
        currentShow: [
          ...current.currentShow,
          {
            id: segmentId ?? createUniqueDomainId("segment", [current.seasonNumber, current.currentWeek, current.currentShow.length + 1, type], current.currentShow.map((segment) => segment.id)),
            type,
            participantIds: [],
  ...
  function revokeChampionship(championshipId: string) {
    setGame((current) => {
      ...
      const event: ChampionshipHistoryEvent = {
        id: createUniqueDomainId("title-revoked", [current.seasonNumber, current.currentWeek, championship.id], current.championshipHistory.map((item) => item.id)),
  ...
  function assignChampionship(championshipId: string, championIds: string[]) {
    setGame((current) => {
      ...
      const event: ChampionshipHistoryEvent = {
        id: createUniqueDomainId("title-assigned", [current.seasonNumber, current.currentWeek, championship.id, ...nextChampionIds], current.championshipHistory.map((item) => item.id)),
        championshipId: championship.id,
        championshipName: championship.name,
        eventType: "assigned",
