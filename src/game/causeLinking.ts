...
function getFinanceReportForResolvedShow(game: GameState, result: ShowResult) {
  return game.financeReports.find((report) => report.resultId === result.id) ?? game.financeReports.find((report) => report.id === `${result.id}-finance`);
}
...
function getTitleHistoryEventsForResolvedShow(game: GameState, result: ShowResult, event?: DurableGameEvent) {
  const directlyLinkedEvents = (game.championshipHistory ?? []).filter((historyEvent) => historyEvent.resultId === result.id || (event?.id !== undefined && historyEvent.eventId === event.id));

  if (directlyLinkedEvents.length) {
    return directlyLinkedEvents;
  }
...
function getRivalryHistoryEventsForResolvedShow(game: GameState, result: ShowResult, event?: DurableGameEvent) {
  const directlyLinkedEvents = (game.rivalryHistory ?? []).filter((historyEvent) => historyEvent.resultId === result.id || (event?.id !== undefined && historyEvent.eventId === event.id));
